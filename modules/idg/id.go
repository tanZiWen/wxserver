...
func NewIdWorker(workerId, datacenterId int64, twepoch int64) (*IdWorker, error) {
    ...
    log.Printf("worker starting. timestamp left shift %d, datacenter id bits %d, worker id bits %d, sequence bits %d, workerid %d", timestampLeftShift, datacenterIdBits, workerIdBits, sequenceBits, workerId)
    return idWorker, nil
}
...
func (id *IdWorker) NextId() (int64, error) {
    ...
    if timestamp < id.lastTimestamp {
        log.Fatalf("clock is moving backwards.  Rejecting requests until %d.", id.lastTimestamp)
...
func (id *IdWorker) NextIds(num int) ([]int64, error) {
    if num > maxNextIdsNum || num < 0 {
        log.Fatalf("NextIds num can't be greater than %d or less than 0", maxNextIdsNum)
