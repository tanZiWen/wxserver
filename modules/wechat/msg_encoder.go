...
func init() {
	sec := setting.Cfg.Section("wechatmp")
	token = sec.Key("TOKEN").MustString("")
	appID = sec.Key("APP_ID").MustString("")
	encodingAESKey = sec.Key("ENCODING_AES_KEY").MustString("")
...
func AesDecrypt(cipherData []byte) ([]byte, error) {
	k := len(aesKey) //PKCS#7
...
func parseEncryptResponse(responseEncryptTextBody []byte) {
	...
	plainText, err := AesDecrypt(cipherData)
	if err != nil {
		fmt.Println(err)
		return
	}
...
func procRequest(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		if encryptType == "aes" {
			...
			// AES Decrypt
			plainData, err := AesDecrypt(cipherData)
			if err != nil {
				fmt.Println(err)
				return
			}
